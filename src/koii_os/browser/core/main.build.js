...
var windows = {
  ...
  getCurrent: function () {
    var lastFocused = windows.openWindows.filter(w => !w.closed).sort((a, b) => b.state.lastFocused - a.state.lastFocused)[0]
    if (lastFocused) {
...
function buildAppMenu (options = {}) {
  var keyMap = userKeyMap(settings.get('keyMap'))

  function getFormattedKeyMapEntry (keybinding) {
    var value = keyMap[keybinding]
...
;
var TouchBar = require('electron').TouchBar
var nativeImage = require('electron').nativeImage
var { TouchBarLabel, TouchBarButton, TouchBarSpacer } = TouchBar
...
var settings = {
  ...
  initialize: function (userDataPath) {
    ...
    try {
      var fileData = fs.readFileSync(settings.filePath, 'utf-8')
      if (fileData) {
...
var llmConfigManager = {
  ...
  loadConfig: function () {
    try {
      if (fs.existsSync(this.configFilePath)) {
        var data = fs.readFileSync(this.configFilePath, 'utf-8')
  ...
  getApiKey: function () {
    ...
    try {
      var decrypted = safeStorage.decryptString(Buffer.from(config.encryptedApiKey, 'hex'))
      return decrypted
    } catch (e) {
  ...
  saveConfig: function (config) {
    ...
    if (config.apiKey && config.apiKey.trim()) {
      try {
        var encrypted = safeStorage.encryptString(config.apiKey)
  ...
  validateConnection: async function (apiUrl, apiKey, modelId) {
    ...
    try {
      var response = await fetch(`${apiUrl}/messages`, {
        method: 'POST',
        headers: {
...
var isInstallerRunning = false
var isDevelopmentMode = process.argv.some(arg => arg === '--development-mode')
var isDebuggingEnabled = process.argv.some(arg => arg === '--debug-browser')
...
var browserPage = 'min://app/index.html'
...
function createWindowWithBounds (bounds, customArgs) {
  ...
  var mainView = new WebContentsView({
    webPreferences: {
...
ipc.handle('validateLLMConnection', async function(e, params) {
  try {
    var result = await llmConfigManager.validateConnection(params.apiUrl, params.apiKey, params.modelId)
    return result
  } catch (err) {
...
ipc.on('tab-state-change', function(e, events) {
  var sourceWindowId = windows.windowFromContents(e.sender)?.id
  if (!sourceWindowId) {
...
ipc.on('request-tab-state', function(e) {
  var otherWindow = windows.getAll().find(w => getWindowWebContents(w).id !== e.sender.id)
  if (!otherWindow) {
...
var placesPage = 'file://' + __dirname + '/js/places/placesService.html'

var placesWindow = null
...
var translatePage = 'min://app/pages/translateService/index.html'
var translatePreload = __dirname + '/pages/translateService/translateServicePreload.js'

app.on('ready', function() {
  ipc.on('page-translation-session-create', function(e) {
    var translateWindow = new BrowserWindow({
      width: 300,
      height: 300,
      show: false,
      webPreferences: {
...
function registerBundleProtocol (ses) {
  ses.protocol.handle('min', (req) => {
    ...
    var pathToServe = path.resolve(__dirname, pathname)
    var relativePath = path.relative(__dirname, pathToServe)
    var isSafe = relativePath && !relativePath.startsWith('..') && !path.isAbsolute(relativePath)
...
function filterPopups (url) {
  ...
  var domain = parser.getUrlHost(url)
...
function removeTrackingParams (url) {
  try {
    ...
    for (var param of urlObj.searchParams) {
...
function handleRequest (details, callback) {
  ...
  var isExceptionDomain = domain && requestDomainIsException(domain)

  var modifiedURL = (enabledFilteringOptions.blockingLevel > 0 && !isExceptionDomain) ? removeTrackingParams(details.url) : details.url
...
function createView (existingViewId, id, webPreferences, boundsString, events) {
  ...
  var viewPrefs = Object.assign({}, getDefaultViewWebPreferences(), webPreferences)
  ...
  events.forEach(function (event) {
    view.webContents.on(event, function (e) {
      ...
      var eventTarget = getWindowFromViewContents(view.webContents) || windows.getCurrent()
    ...
    */
    ...
    return {
      ...
      createWindow: function (options) {
        var view = new WebContentsView({ webPreferences: getDefaultViewWebPreferences(), webContents: options.webContents })
  ...
  */
  view.webContents.on('did-start-navigation', function (event) {
    if (event.isMainFrame && !event.isSameDocument) {
      var hasJS = viewStateMap[id].hasJS
      var shouldHaveJS = (!(settings.get('filtering')?.contentTypes?.includes('script'))) || event.url.startsWith('min://')
      if (hasJS !== shouldHaveJS) {
        setTimeout(function () {
          view.webContents.stop()
          var currentWindow = getWindowFromViewContents(view.webContents)
          destroyView(id)
          var newView = createView(existingViewId, id, Object.assign({}, webPreferences, { javascript: shouldHaveJS }), boundsString, events)
...
function setView (id, senderContents) {
  var win = windows.windowFromContents(senderContents).win
...
function hideCurrentView (senderContents) {
  var win = windows.windowFromContents(senderContents).win
  var currentId = windows.getState(win).selectedView
  if (currentId) {
...
function getWindowFromViewContents (webContents) {
  var viewId = Object.keys(viewMap).find(id => viewMap[id].webContents === webContents)
...
ipc.on('setView', function (e, args) {
  ...
  if (args.focus && BrowserWindow.fromWebContents(e.sender) && BrowserWindow.fromWebContents(e.sender).isFocused()) {
    var couldFocus = focusView(args.id)
    if (!couldFocus) {
      e.sender.focus()
    }
  }
...
ipc.handle('getNavigationHistory', function (e, id) {
  ...
  var entries = []
  var activeIndex = viewMap[id].webContents.navigationHistory.getActiveIndex()
  var size = viewMap[id].webContents.navigationHistory.length()
...
function downloadHandler (event, item, webContents) {
  var sourceWindow = windows.windowFromContents(webContents)?.win
  if (!sourceWindow) {
...
function listenForDownloadHeaders (ses) {
  ses.webRequest.onHeadersReceived(function (details, callback) {
    if (details.resourceType === 'mainFrame' && details.responseHeaders) {
      ...
      var isFileView = typeHeader instanceof Array && !typeHeader.some(t => t.includes('text/html'))
    ...
    */
    if (details.webContents && (details.webContents.getURL().startsWith('min://app/pages/pdfViewer') || details.webContents.getURL().startsWith('min://app/reader/') || details.webContents.getURL() === 'min://app/index.html')) {
      var filteredHeaders = Object.fromEntries(
...
var defaultUserAgent = app.userAgentFallback
...
function getFirefoxUA () {
  ...
  */
  ...
  var fxVersion = 91 + Math.floor((Date.now() - 1628553600000) / (4.1 * 7 * 24 * 60 * 60 * 1000))
...
function enableGoogleUASwitcher (ses) {
  ses.webRequest.onBeforeSendHeaders((details, callback) => {
    ...
    var chromiumVersion = process.versions.chrome.split('.')[0]
...
function createPrompt (options, callback) {
  ...
  var { parent, width = 360, height = 140 } = options
...
ipc.handle('showOpenDialog', async function (e, options) {
  var result = await dialog.showOpenDialog(windows.windowFromContents(e.sender).win, options)
  return result.filePaths
})

ipc.handle('showSaveDialog', async function (e, options) {
  var result = await dialog.showSaveDialog(windows.windowFromContents(e.sender).win, options)
...
var safeStorage = require('electron').safeStorage
var passwordFilePath = path.join(userDataPath, 'passwordStore')
...
function credentialStoreSetPasswordBulk (accounts) {
  var fileContent = readSavedPasswordFile()
...
function credentialStoreSetPassword (account) {
  ...
  for (var i = 0; i < fileContent.credentials.length; i++) {
